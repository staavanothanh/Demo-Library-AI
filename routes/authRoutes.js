...
function createAuthRoutes({ controller, passport, renderForm, showValidation }) {
  ...
  router.post("/register", [body("username").trim().isLength({ min: 3, max: 30 }).withMessage("Username must be 3–30 characters."), body("password").isLength({ min: 8 }).withMessage("Password must contain at least 8 characters."), showValidation("register")], controller.register);
